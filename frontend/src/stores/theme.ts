...
const ICON_STYLE_KEY = 'community_icon_style'
...
function applyTheme(
  ...
  document.documentElement.dataset.icon = iconStyle
}

// 网站样式：主题颜色（蓝/红/绿）+ 颜色模式（浅色/深色/跟随系统）+ 全站字体大小 + 图标风格
export const useThemeStore = defineStore('theme', () => {
  ...
  const iconStyle = ref<IconStyle>(
    (localStorage.getItem(ICON_STYLE_KEY) as IconStyle) || 'outline',
  )

  // 跟随系统时，监听系统深浅变化自动切换
  DARK_QUERY.addEventListener('change', () => {
    if (colorMode.value === 'system') {
      applyTheme(themeColor.value, colorMode.value, fontSize.value, iconStyle.value)
    }
  })

  // 选中后立即生效，并记住用户选择
  watch([themeColor, colorMode, fontSize, iconStyle], () => {
    applyTheme(themeColor.value, colorMode.value, fontSize.value, iconStyle.value)
    ...
    localStorage.setItem(ICON_STYLE_KEY, iconStyle.value)
  })
  applyTheme(themeColor.value, colorMode.value, fontSize.value, iconStyle.value)

  return { themeColor, colorMode, fontSize, iconStyle }
})
